...
| `sentence-transformers` | Tier 0/1/3/UX и source analytics | Эмбеддинги для вопросов/ответов/чанков |
...
В текущей архитектуре real-user вопросы **не используются как benchmark-датасет**
для Tier 1/2/3. Оценка retrieval/generation/e2e выполняется на synthetic и manual
датасетах, где есть управляемый и проверяемый ground truth.
...
3. **Domain Analysis** — поведенческие и лингвистические паттерны
   пользовательских вопросов.

### 7.2 Почему это корректно методологически

Для реальных вопросов обычно отсутствует полноценная эталонная разметка
по релевантным документам и по корректному ответу. Поэтому их применение в
качестве строгого benchmark-датасета приводит к смешению операционных сигналов
и «золотых» метрик качества.
