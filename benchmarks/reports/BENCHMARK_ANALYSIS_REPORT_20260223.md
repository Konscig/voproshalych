...
**Примечание методологического характера:** В рамках анализа утилизации было обработано 251 реальных пользовательских вопросов. Данное значение отличается от общего количества вопросов в корпусе (5,248), что обусловлено ограничениями на параметр `question_limit` в конфигурации запуска бенчмарка. Тем не менее, полученная выборка является репрезентативной для оценки паттернов использования базы знаний.
...
#### 6.3.1. Детальный анализ структуры утилизации

Анализ показывает, что при top-k = 10 система в среднем использует 177 уникальных чанков из 202 доступных. Данное соотношение свидетельствует о достаточно высоком перекрытии запросов — различные вопросы пользователей приводят к извлечению одних и тех же информационных фрагментов. С одной стороны, это указывает на наличие ядра базы знаний, которое покрывает основные потребности пользователей. С другой стороны, 12.38% неиспользуемых чанков могут содержать ценную информацию, которая по тем или иным причинам не извлекается при текущей конфигурации поиска.

#### 6.3.2. Рекомендации по оптимизации утилизации

1. **Аудит неиспользуемых чанков:** необходимо провести систематический анализ 25 невостребованных чанков для определения причин их неиспользования — будь то избыточность дублирующейся информации, некачественный контент или некорректное разбиение на чанки.

2. **Оптимизация top-k параметра:** текущее значение top-k = 10 обеспечивает хороший охват, однако возможно дальнейшее исследование влияния данного параметра на качество ответов и утилизацию.

3. **Расширение выборки:** для более точной оценки утилизации рекомендуется провести анализ на полном корпусе вопросов (5,248 вопросов) без ограничений по question_limit.
...
**Примечание методологического характера:** В рамках анализа тематического покрытия использовалась выборка из 251 вопроса. Данное ограничение обусловлено конфигурацией аналитического запуска и не отражает полного объёма пользовательского корпуса. При расширении анализа на совокупность в 5,248 вопросов структура тематических кластеров может претерпеть существенную трансформацию.
...
#### 7.3. Детальный анализ тематических кластеров

Тематическая структура выявленных кластеров демонстрирует характерное для системы поддержки студентов распределение. Наиболее многочисленные кластеры связаны с административными процедурами (пропуска, студенческие билеты), организацией учебного процесса (мероприятия, физкультура) и информационным обеспечением (общие вопросы об университете). Примечательно наличие кластера с низким количеством вопросов (topic_id = 3, всего 8 вопросов), содержащего благодарности и не относящиеся к делу высказывания — данный кластер требует особого внимания при разработке системы фильтрации нерелевантных запросов.

Среднее значение уникальных чанков на тему (25.2) указывает на умеренную концентрацию информационных источников вокруг каждой тематической области. Данный показатель свидетельствует о достаточно полном покрытии базой знаний основных предметных направлений, при этом оставляя пространство для расширения в случае необходимости.

#### 7.4. Выводы по тематическому покрытию

1. **Адекватное покрытие основных тем:** система охватывает ключевые направления студенческих запросов — от военкомата до физкультуры;
2. **Необходимость расширения выборки:** текущий анализ проведён на 251 вопросе, что требует валидации на полном корпусе;
3. **Наличие оффтопик-кластеров:** выявлены группы нерелевантных запросов, требующие обработки.
...
#### 8.3.3. Детальный анализ показателей охвата

Корпус реальных пользовательских запросов в количестве 5,248 единиц представляет собой репрезентативную выборку для анализа поведенческих паттернов и предметной области системы. Показатель with_answers_rate, равный 57.91%, свидетельствует о том, что значительная доля запросов (42.09%, или 2,209 вопросов) не получила ответа от системы. Данное соотношение указывает на наличие существенного резерва для улучшения охвата базы знаний.

Распределение оценок качества демонстрирует выраженную поляризацию: 252 вопроса получили максимальную оценку «5» (43.75% от оценённых), тогда как 323 вопроса оценены минимальной оценкой «1» (56.25%). Единственный случай оценки «4» (всего 1 вопрос) указывает на отсутствие плавного перехода между категориями — система либо удовлетворяет запрос полностью, либо не удовлетворяет вовсе. Данный паттерн требует детального исследования причин бинарного характера оценок.

#### 8.3.4. Лингвистический анализ

Лексический состав корпуса вопросов характеризуется преобладанием вопросительных конструкций, что типично для систем поддержки. Токен «как» с частотностью 1,137 (21.67% от общего числа вопросов) является доминирующим и свидетельствует о преимущественно процедурном характере запросов — пользователи стремятся узнать способ выполнения тех или иных действий.

Присутствие контекстуального маркера «тюмгу» (262 вхождения) подтверждает релевантность системы её целевой аудитории. Сленговые маркеры «препод» (61), «сессия» (28), «пересдача» (19) указывают на необходимость расширенной обработки неформальной лексики в запросах пользователей.
