...
class LLMJudge:
    ...
    def generate_question_from_chunk(self, chunk_text: str) -> Dict[str, str]:
        ...
        """
        prompt = """Ты - студент университета. Твоя задача - сформулировать
вопрос в поддержку на основе текста ниже.

Критически важно:
- Избегай канцелярщины и формального стиля (не пиши "Каков регламент...?").
- Используй естественный разговорный русский язык.
- Допускается умеренный сленг (препод, зачетка, хвост).
- Вопрос должен звучать так, как будто его быстро пишут в мессенджере с телефона.
- Моделируй разные состояния: спешка, непонимание, уточнение.
- Редко добавляй лёгкий шум: небольшая опечатка или неидеальная формулировка.
- Вопрос должен быть однозначно отвечаем по тексту чанка.

Пример: вместо "Каков регламент сдачи?" пиши
"А когда вообще сдавать надо, есть инфа?"

После вопроса сформируй краткий идеальный ответ строго по фактам из текста.
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "Пиши так, как реальный студент пишет в чат поддержки: "
                            "кратко, разговорно, иногда со сленгом и лёгкой небрежностью."
                        ),
                    },
